...
class User {
	protected static  $tblname = "tbluser";
	protected static  $innertbl = " tu INNER JOIN tblroluser tru ON tu.id_rol_user = tru.id_rol_user ";
	...
	function find_user($id="",$user_name=""){
		global $mydb_user;
		$mydb_user->setQuery("SELECT * FROM ".self::$tblname.self::$innertbl." 
			WHERE id_user = {$id} OR user_nombre = '{$user_name}'");
		$cur = $mydb_user->executeQuery();
	...
	static function userAuthentication($user_nom,$user_contra){
		global $mydb_user;
		$mydb_user->setQuery("SELECT * FROM tbluser WHERE user_nombre = '". $user_nom ."' and user_contra = '". $user_contra ."'");
		$cur = $mydb_user->executeQuery();
		if($cur==false){
			die(mysql_error());
		}
		$row_count = $mydb_user->num_rows($cur);//get the number of count
		 ...
		 $user_found = $mydb_user->loadSingleResult();
		 	$_SESSION['id_user']   		= $user_found->id_user;
		 	$_SESSION['user_nombre'] 	= $user_found->user_nombre;
		 	$_SESSION['user_contra'] 		= $user_found->user_contra;
		 	$_SESSION['id_rol_user'] 		= $user_found->id_rol_user;
		   return true;
		 }else{
		 	return false;
		 }
	...
	function single_user_rol($id_rol_user=""){
		global $mydb_user;
		$mydb_user->setQuery("SELECT * FROM ".self::$tblname." 
			Where id_rol_user= '{$id_rol_user}' LIMIT 1");
		$cur = $mydb_user->loadSingleResult();
		return $cur;
	}
